...
// ===== USERS ROUTES (ADICIONAR NO BACKEND) =====
app.get('/api/users', async (req, res) => {
  try {
    const [rows] = await pool.execute('SELECT id, username, fullName, role, createdAt FROM users ORDER BY fullName, username');
    res.json(rows);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
  ...
  try {
    ...
    await pool.execute(`
      ...
      ON DUPLICATE KEY UPDATE
        ...
        password = CASE WHEN VALUES(password) != '' THEN VALUES(password) ELSE password END,
        role = VALUES(role)
...
app.put('/api/users/:id', async (req, res) => {
  try {
    ...
    let updateQuery = 'UPDATE users SET fullName = ?, role = ?';
    let params = [user.fullName, user.role];
...
// ===== ACCOUNTS PAYABLE ROUTES (ADICIONAR NO BACKEND) =====
...
app.post('/api/accounts-payable', async (req, res) => {
  try {
    ...
    await pool.execute(`
      INSERT INTO accounts_payable (id, name, amount, dueDate, isPaid, notes)
      VALUES (?, ?, ?, ?, ?, ?)
      ON DUPLICATE KEY UPDATE
        name = VALUES(name),
        ...
        notes = VALUES(notes)
    `, [id, entry.name, entry.amount, entry.dueDate, entry.isPaid, entry.notes]);
    ...
    for (let i = 0; i < installments; i++) {
      ...
      await pool.execute(`
        INSERT INTO accounts_payable (
          id, name, amount, dueDate, isPaid, notes, seriesId, 
          totalInstallmentsInSeries, installmentNumberOfSeries, createdAt
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [
        entry.id, entry.name, entry.amount, entry.dueDate, entry.isPaid,
        entry.notes, entry.seriesId, entry.totalInstallmentsInSeries,
        entry.installmentNumberOfSeries, entry.createdAt
      ]);
...
// ===== SUPPLIERS ROUTES (ADICIONAR NO BACKEND) =====
...
app.delete('/api/suppliers/:id', async (req, res) => {
  try {
    const { id } = req.params;
    // Delete related records first due to CASCADE
...
// Recreate schema
// Quotes table (ADICIONAR NO createAllTables)
await pool.execute(`
  CREATE TABLE IF NOT EXISTS quotes (
    id VARCHAR(36) PRIMARY KEY,
    quoteNumber VARCHAR(50) UNIQUE NOT NULL,
