...
// ✅ CORREÇÃO: Função helper segura para toFixed
const safeToFixed = (value: any, decimals: number = 2): string => {
  const num = parseFloat(value);
  return isNaN(num) ? '0.00' : num.toFixed(decimals);
};

// ✅ CORREÇÃO: Função helper segura para números
  ...
  const generateOrderServicePdf = (quoteDataForOs: Quote) => {
    ...
    const tableBody = quoteDataForOs.items.map(item => {
        ...
        if (item.pricingModel === PricingModel.PER_SQUARE_METER) {
            // ✅ CORREÇÃO: Usar safeNumber e safeToFixed
            const safeQuantity = safeNumber(item.quantity);
            qtyDisplay = `${safeToFixed(safeQuantity, 2)} m²`;
            
            const safeWidth = safeNumber(item.width);
            const safeHeight = safeNumber(item.height);
            const safeItemCount = safeNumber(item.itemCountForAreaCalc);
            
            if (safeWidth > 0 && safeHeight > 0 && safeItemCount > 0) {
                qtyDisplay += ` (${safeToFixed(safeWidth, 2)}m x ${safeToFixed(safeHeight, 2)}m x ${safeItemCount}pç)`;
            }
        } else {
            ...
            qtyDisplay = `${safeNumber(item.quantity)} ${unitSuffix}`;
        }
        return [
            item.productName,
            qtyDisplay,
    ...
    doc.setFontSize(10); doc.setFont('helvetica', 'normal');
    doc.text(`Subtotal: ${formatCurrency(safeNumber(quoteDataForOs.subtotal))}`, pageWidth - margin, yPos, { align: 'right' }); yPos += 6;
    if (safeNumber(quoteDataForOs.discountAmountCalculated) > 0) {
      doc.text(`Desconto Aplicado: ${formatCurrency(safeNumber(quoteDataForOs.discountAmountCalculated))}`, pageWidth - margin, yPos, { align: 'right' }); yPos += 6;
    }
    doc.setFontSize(10); doc.setFont('helvetica', 'bold');
    doc.text(`Subtotal com Desconto: ${formatCurrency(safeNumber(quoteDataForOs.subtotalAfterDiscount))}`, pageWidth - margin, yPos, { align: 'right' }); yPos += 6;
    doc.setFontSize(10); doc.setFont('helvetica', 'normal');
    ...
    const osEffectiveTotalBeforeCredit = (quoteDataForOs.selectedPaymentMethod && quoteDataForOs.selectedPaymentMethod.toLowerCase().includes('cartão')) 
                                    ? safeNumber(quoteDataForOs.totalCard)
                                    : safeNumber(quoteDataForOs.totalCash);
    const osCreditApplied = safeNumber(quoteDataForOs.downPaymentApplied);
    ...
    if (quoteDataForOs.selectedPaymentMethod?.toLowerCase().includes('cartão')) {
        doc.text(`Total (Cartão - Antes do Sinal): ${formatCurrency(safeNumber(quoteDataForOs.totalCard))}`, pageWidth - margin, yPos, { align: 'right' }); yPos += 6;
    } else {
        doc.text(`Total (À Vista - Antes do Sinal): ${formatCurrency(safeNumber(quoteDataForOs.totalCash))}`, pageWidth - margin, yPos, { align: 'right' }); yPos += 6;
    }
      ...
      <div className="bg-gray-800 p-6 md:p-8 rounded-lg shadow-2xl w-full max-w-3xl max-h-[95vh] overflow-y-auto text-gray-300">
              ...
              <tbody className="bg-gray-800 divide-y divide-gray-600">
                {items.map((item, index) => {
                  ...
                  <tr key={index}>
                    ...
                    <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-300 text-right">{formatCurrency(safeNumber(item.unitPrice))}</td>
                    <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-300 text-right">{formatCurrency(safeNumber(item.totalPrice))}</td>
                  </tr>
                )})}
              </tbody>
            </table>
        ...
        <div className="mb-6 bg-gray-700 p-4 rounded-md text-sm">
            ...
            <div className="space-y-1">
                <div className="flex justify-between"><span>Subtotal (base):</span><span>{formatCurrency(safeNumber(subtotal))}</span></div>
                {safeNumber(discountAmountCalculated) > 0 && (
                <>
                    <div className="flex justify-between">
                        <span>Desconto ({discountType === 'percentage' ? `${safeNumber(discountValue)}%` : `${formatCurrency(safeNumber(discountValue))}`}):</span>
                        <span className="text-red-400">- {formatCurrency(safeNumber(discountAmountCalculated))}</span>
                    </div>
                </>
                )}
                <div className="flex justify-between font-semibold">
                    <span>Subtotal com Desconto:</span>
                    <span>{formatCurrency(safeNumber(subtotalAfterDiscount))}</span>
                </div>
                <hr className="my-1 border-gray-600"/>
                <div className="flex justify-between">
                    <span className="text-gray-400">Total (À Vista - antes do sinal):</span>
                    <span className="text-gray-400">{formatCurrency(safeNumber(totalCash))}</span>
                </div>
                <div className="flex justify-between">
                    <span className="text-gray-400">Total (Cartão - antes do sinal):</span>
                    <span className="text-gray-400">{formatCurrency(safeNumber(totalCard))}</span>
                </div>

                {safeNumber(creditActuallyAppliedToThisQuote) > 0 && (
                     <div className="flex justify-between text-blue-400">
                        <span>Sinal/Haver Utilizado:</span>
                        <span>- {formatCurrency(safeNumber(creditActuallyAppliedToThisQuote))}</span>
                    </div>
                )}
                <hr className="my-1 border-gray-600"/>
                <div className="flex justify-between text-lg font-bold">
                    <span className="text-gray-100">VALOR FINAL PAGO/A PAGAR:</span>
                    <span className="text-yellow-400">{formatCurrency(safeNumber(finalAmountPaidOrDue))}</span>
                </div>
